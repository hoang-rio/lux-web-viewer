...
from . import db
from . import mobile
from . import modbus_routes
from . import notifications
from . import security
from . import settings_routes
from . import streaming
from . import trigger_routes
from . import tuya_routes
...
def create_runner():
    ...
    app.add_routes([
        ...
        web.get("/has-admin-access", security.has_admin_access),
        web.post("/settings", settings_routes.update_settings),
        web.get("/tuya-devices", tuya_routes.get_tuya_devices),
        web.get("/tuya-devices/mappings", tuya_routes.get_device_mappings),
        web.post("/tuya-devices/scan", tuya_routes.scan_tuya_devices),
        web.get("/tuya-devices/wizard-status", tuya_routes.tuya_wizard_status),
        web.post("/tuya-devices", tuya_routes.add_tuya_device),
        web.put("/tuya-devices/{id}", tuya_routes.update_tuya_device),
        web.delete("/tuya-devices/{id}", tuya_routes.delete_tuya_device),
        web.post("/tuya-devices/{id}/status", tuya_routes.get_device_status),
        web.post("/tuya-devices/batch-status", tuya_routes.batch_device_status),
        web.post("/tuya-devices/{id}/control", tuya_routes.control_device),
        web.get("/triggers", trigger_routes.get_triggers),
        web.post("/triggers", trigger_routes.save_trigger_route),
        web.delete("/triggers/{id}", trigger_routes.delete_trigger_route),
        web.post("/triggers/{id}/test", trigger_routes.test_trigger_route),
        web.get("/triggers/{id}/history", trigger_routes.get_trigger_history_route),
        web.get("/modbus/status", modbus_routes.modbus_status),
        web.get("/modbus/registers", modbus_routes.modbus_registers_route),
        web.get("/modbus/read", modbus_routes.modbus_read),
        web.post("/modbus/write", modbus_routes.modbus_write),
