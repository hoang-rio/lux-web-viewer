...
const ModbusDashboard = ({ onClose, inverterId }: ModbusDashboardProps) => {
  ...
  const inverterQuery = inverterId ? `?inverter_id=${encodeURIComponent(inverterId)}` : '';
  ...
  const fetchAll = useCallback(async (quiet = false) => {
    ...
    setMessage(null);
    try {
      const regsRes = await apiFetch('/modbus/registers', { withAuth: true });
      if (!regsRes.ok) {
        setNoAccess(true);
        return;
      }
      ...
      const readRes = await apiFetch(`/modbus/read${inverterQuery}`, { withAuth: true });
  ...
  const applyWrite = async (item: IModbusItem, value: number | string, confirm: boolean) => {
    ...
    setMessage(null);
    try {
      const body: Record<string, unknown> = { key: item.key, value, confirm: Boolean(confirm) };
      if (inverterId) body.inverter_id = inverterId;
      const res = await apiFetch('/modbus/write', {
        method: 'POST',
